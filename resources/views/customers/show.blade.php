                ...
                <div class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="bg-sage/10 rounded-md p-4 border border-ink/15">
                            <span class="text-xs text-ink/50 font-medium uppercase tracking-wider">{{ __('Segment') }}</span>
                            @php
                                $segmentColors = [
                            ...
                            @endphp
                            <div class="mt-1">
                                <span class="inline-flex px-2.5 py-0.5 rounded-full text-xs font-bold {{ $segmentColors[$customer->customer_type] ?? 'bg-ink/5' }}">
                                    {{ $customer->customer_type }}
                                </span>
                            </div>
                        </div>
                        <div class="bg-sage/10 rounded-md p-4 border border-ink/15">
                            <span class="text-xs text-ink/50 font-medium uppercase tracking-wider">{{ __('N° Document') }}</span>
                            <div class="text-base font-bold text-ink mt-1">{{ $customer->document_id }}</div>
                        </div>
                        <div class="bg-sage/10 rounded-md p-4 border border-ink/15">
                            <span class="text-xs text-ink/50 font-medium uppercase tracking-wider">{{ __('ICE') }}</span>
                            <div class="text-base font-bold text-ink mt-1">{{ $customer->ice ?? '-' }}</div>
                        </div>
                        <div class="bg-sage/10 rounded-md p-4 border border-ink/15">
                            <span class="text-xs text-ink/50 font-medium uppercase tracking-wider">{{ __('Nom') }}</span>
                            <div class="text-base font-bold text-ink mt-1">{{ $customer->name }}</div>
                        </div>
                        <div class="bg-sage/10 rounded-md p-4 border border-ink/15">
                            <span class="text-xs text-ink/50 font-medium uppercase tracking-wider">{{ __('Email') }}</span>
                            <div class="text-base font-bold text-ink mt-1">{{ $customer->email }}</div>
                        </div>
                        <div class="bg-sage/10 rounded-md p-4 border border-ink/15">
                            <span class="text-xs text-ink/50 font-medium uppercase tracking-wider">{{ __('Téléphone') }}</span>
                            <div class="text-base font-bold text-ink mt-1">{{ $customer->phone }}</div>
                        </div>
                        <div class="bg-sage/10 rounded-md p-4 border border-ink/15 md:col-span-2">
                            ...
                            <div class="text-base font-bold text-ink mt-1">{{ $customer->address }}</div>
                        </div>
                    </div>

                    <div class="flex justify-end space-x-3 border-t border-ink/15 pt-6">
                        <a href="{{ route('customers.sales', $customer) }}" class="inline-flex items-center px-5 py-2.5 btn-secondary text-cream font-semibold rounded-md text-sm transition">
