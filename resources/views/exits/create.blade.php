<x-app-layout>
 ...
 {{ $customer->name }}
                        ...
                        <div>
                            ...
                            <input type="number" name="quantity" id="quantity" min="1" required value="{{ old('quantity') }}" oninput="calculateTotal()" class="mt-1 block w-full border-gray-300 rounded-xl shadow-sm focus:ring-blue-500 focus:border-blue-500 transition duration-150">
                            @error('quantity')
                                <span class="text-red-500 text-xs mt-1 block font-medium">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="unit_price" id="unit_price" step="0.01" min="0" required value="{{ old('unit_price') }}" oninput="calculateTotal()" class="mt-1 block w-full border-gray-300 rounded-xl shadow-sm focus:ring-blue-500 focus:border-blue-500 transition duration-150">
                            @error('unit_price')
                                <span class="text-red-500 text-xs mt-1 block font-medium">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div>
                            <label for="paid_amount" class="block text-sm font-semibold text-gray-700 mb-1">Montant Payé Initialement (DH)</label>
                            <input type="number" name="paid_amount" id="paid_amount" step="0.01" min="0" required value="{{ old('paid_amount', 0) }}" oninput="adjustPaymentStatus()" class="mt-1 block w-full border-gray-300 rounded-xl shadow-sm focus:ring-blue-500 focus:border-blue-500 transition duration-150">
                            @error('paid_amount')
                                <span class="text-red-500 text-xs mt-1 block font-medium">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="document" id="document" required value="{{ old('document', 'BL-' . strtoupper(Str::random(6))) }}" class="mt-1 block w-full border-gray-300 rounded-xl shadow-sm focus:ring-blue-500 focus:border-blue-500 transition duration-150">
                            @error('document')
                                <span class="text-red-500 text-xs mt-1 block font-medium">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="bg-gray-50 rounded-2xl p-5 border border-gray-100 mb-6">
                        <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Récapitulatif Financier BI</h4>
                        <div class="grid grid-cols-3 gap-4 text-center">
                            <div>
                                <span class="text-[11px] font-semibold text-gray-400 block">Total Vente</span>
                                <span id="totalLabel" class="text-base font-black text-gray-900">0,00 DH</span>
                            </div>
                            <div>
                                <span class="text-[11px] font-semibold text-gray-400 block">Montant Payé</span>
                                <span id="paidLabel" class="text-base font-black text-emerald-600">0,00 DH</span>
                            </div>
                            <div>
                                <span class="text-[11px] font-semibold text-gray-400 block">Solde Restant</span>
                                <span id="remainingLabel" class="text-base font-black text-red-500">0,00 DH</span>
                            </div>
                        </div>
                    </div>
