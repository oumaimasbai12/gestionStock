    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="page-subtitle">Gérez et suivez les approvisionnements de stock reçus des fournisseurs.</p>
            </div>
            <div class="flex items-center space-x-3">
                @if(!auth()->user()->hasRole('site_manager'))
                    <a href="{{ route('entries.trash') }}" class="inline-flex items-center bg-sage/10 hover:bg-ink/5 text-accent border border-ink/20 px-4 py-2 rounded-md text-sm font-medium transition space-x-1.5">
                ...
                @endif
                <a href="{{ route('entries.create') }}" class="inline-flex items-center btn-primary text-cream px-4 py-2 rounded-md text-sm font-semibold transition space-x-1.5">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <tbody class="text-sm text-ink/80 divide-y divide-ink/10">
                            @forelse($entries as $entry)
                                <tr class="hover:bg-accent/5 transition duration-150">
                                    ...
                                    <td class="py-4 px-6">
                                        @if($entry->chantier)
                                            <span class="inline-flex items-center text-xs font-semibold bg-accent/15 text-ink border border-accent/30 px-2 py-0.5 rounded-md">
 {{ $entry->chantier->name }}
                                            </span>
                                        @else
                                    ...
                                    <td class="py-4 px-6 text-right">
                                        <div class="inline-flex items-center space-x-2">
                                            <a href="{{ route('entries.show', $entry) }}" class="inline-flex items-center bg-sage/10 hover:bg-ink/5 text-ink border border-ink/20 px-3 py-1.5 rounded-md text-xs font-semibold transition space-x-1" title="Voir les détails">
                                            ...
                                            @if(!auth()->user()->hasRole('site_manager'))
                                                <a href="{{ route('entries.edit', $entry) }}" class="inline-flex items-center bg-accent/15 hover:bg-accent/20 text-ink border border-accent/30 px-3 py-1.5 rounded-md text-xs font-semibold transition space-x-1" title="Modifier">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                ...
                                                <form action="{{ route('entries.destroy', $entry) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir archiver cette entrée de stock ?');" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="inline-flex items-center bg-accent/15 hover:bg-accent/20 text-ink border border-accent/30 px-3 py-1.5 rounded-md text-xs font-semibold transition space-x-1" title="Archiver l'entrée">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5-4h4a1 1 0 011 1v2H9V4a1 1 0 011-1z"/>
                                                        </svg>
                                                        <span>Archiver</span>
